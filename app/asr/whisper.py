    ...
    *,
    ...
    while True:
        ...
        try:
            ...
            initial_prompt = (str(initial_prompt).strip() if initial_prompt else None)
            res = asr.transcribe(pcm16_bytes=pcm, sample_rate=sr, language=lang, initial_prompt=initial_prompt)
...
class IsolatedWhisperAsr:
    ...
    def transcribe(
        ...
        *,
        ...
        if self._cpu_fallback is not None:
            return self._cpu_fallback.transcribe(
        ...
        if not self._ensure_worker():
            try:
                ...
                pass
            return self._fallback_cpu().transcribe(
                pcm16_bytes=pcm16_bytes,
                sample_rate=sample_rate,
                language=language,
                initial_prompt=initial_prompt,
